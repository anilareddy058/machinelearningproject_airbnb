...
#samplerownums<- sample(200,50)
(Size <- (as.integer(nrow(listing)/4)))  ## Test will be 1/4 of the data
(SAMPLE <- sample(nrow(listing), Size))
listing_testset <-listing[SAMPLE, columns]
...
listing_trainset <- listing[-SAMPLE, columns]
...
SVM_fit2 <- svm(listing_trainset$price_level~., data=listing_trainset, kernel="linear", cost=10)
print(SVM_fit2)
#plot(SVM_fit2, listing_trainset)
(pred_2 <- predict(SVM_fit2, listing_testset, type="class")) 
(pred_2) 
...
#radial
library(e1071)
SVM_fit3<- svm(listing_trainset$price_level~., data=listing_trainset, kernel = 'radial', cost=0.1)
print(SVM_fit3)
...
SVM_fit3<- svm(listing_trainset$price_level~., data=listing_trainset, kernel = 'radial', cost=1)
print(SVM_fit3)
#plot(SVM_fit3, listing_trainset)
(pred_3 <- predict(SVM_fit3, listing_testset, type="class")) 
(pred_3) 
...
SVM_fit3<- svm(listing_trainset$price_level~., data=listing_trainset, kernel = 'radial', cost=10)
print(SVM_fit3)
#plot(SVM_fit3, listing_trainset)
(pred_3 <- predict(SVM_fit3, listing_testset, type="class")) 
(pred_3) 
